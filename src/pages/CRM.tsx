...
function DraggableLeadCard({
  ...
  return (
    <Card
      ref={setNodeRef}
      style={style}
      className={`border-2 hover:shadow-lg transition-all cursor-grab active:cursor-grabbing animate-scale-in ${isDragging ? "shadow-2xl ring-2 ring-primary" : ""} ${isSelected ? "border-primary ring-2 ring-primary/60" : ""}`}
      ...
      <CardContent className="p-3 md:p-4">
        <div className="flex items-start gap-2 mb-2">
          <div onClick={(e) => e.stopPropagation()}>
            <Checkbox
              checked={isSelected}
              onCheckedChange={(checked) => onToggleSelect(lead.id, checked === true)}
              className="mt-1"
        ...
        <div className="flex gap-2 mb-2">
          <Button
            size="sm"
            variant="outline"
            className="flex-1 h-9 md:h-8 text-xs"
            onClick={(e) => {
              e.stopPropagation();
        ...
        <div className=" flex justify-between">
          ...
          <span className="text-xs text-muted-foreground mt-1">
            Last contacted: {lastContactedAt || lead.lastContact || "Not yet"}
          </span>
        </div>
      </CardContent>
    </Card>
...
function DroppableStage({
  ...
  return (
    <Card
      className={`shadow-card animate-slide-up transition-all flex flex-col max-h-[80vh] overflow-hidden ${isOver ? "ring-2 ring-primary" : ""}`}
      style={{
    ...
    >
      <CardHeader className={`${stage.color} text-white rounded-t-lg`}>
        <div className="flex flex-col gap-2">
          <CardTitle className="text-base md:text-lg flex items-center justify-between">
            ...
            <Badge className="bg-white/20 text-white border-white/30 text-xs md:text-sm">{leads.length}</Badge>
          </CardTitle>
          <div className="flex items-center gap-3 text-xs">
            <button
              className="text-white/80 hover:text-white transition-colors underline-offset-2 hover:underline disabled:opacity-50"
              onClick={(e) => {
                e.preventDefault();
      ...
      <CardContent
        ref={setNodeRef}
        className={`p-3 md:p-4 space-y-3 min-h-[300px] md:min-h-[400px] flex-1 overflow-y-auto transition-colors ${isOver ? "bg-accent/50" : ""}`}
      >
        {leads.map((lead, index) => (
          <DraggableLeadCard
            ...
            onEdit={onEdit}
            onContact={onContact}
            lastContactedAt={lastContactTimes[lead.id]}
  ...
  const { data: leads = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select("*")
        .eq("user_id", user?.id)
        .in("stage", ["new", "contacted", "qualified", "converted", "lost"])
        .order("created_at", {
          ascending: false,
        });
      ...
      <div className="p-4 md:p-8 space-y-6 md:space-y-8 animate-fade-in">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <div className="flex gap-2">
            ...
            <Button
              className="gradient-primary text-primary-foreground shadow-primary w-full sm:w-auto"
              ...
              >
                <CardContent className="p-4 md:p-6">
                  <div className="flex items-center justify-between">
                    ...
                    <div
                      className={`h-10 w-10 md:h-12 md:w-12 rounded-full ${stage.color}/10 flex items-center justify-center`}
                    >
        ...
        {hasSelection && (
          <Card className="shadow-card border border-primary/40">
          ...
          <div className="grid gap-4 md:gap-6 grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
            {stages.map((stage, stageIndex) => (
              <DroppableStage
                ...
                lastContactTimes={lastContactTimes}
                selectedLeadIds={selectedLeads}
                onToggleSelect={handleToggleLeadSelection}
                onToggleStageSelect={handleToggleStageSelection}
              />
            ))}
          </div>
          <DragOverlay>
